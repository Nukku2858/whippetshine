...
const packages: Package[] = [
  { name: "Small Home", price: 250, description: "Single-story homes — up to 1,500 sq ft.", stripePriceId: "price_1T4BnYQ47JXIZZAQbOf4tty8" },
  { name: "Standard Home", price: 400, description: "Two-story homes — 1,500–2,500 sq ft.", popular: true, stripePriceId: "price_1T4BnkQ47JXIZZAQlvrXANCs" },
  { name: "Large Home", price: 600, description: "Large & multi-story homes — 2,500+ sq ft.", stripePriceId: "price_1T4BnxQ47JXIZZAQRJio6Mid" },
];

const addOns = [
  { id: "gutter-soffit", name: "Gutter face & soffit wash", addonPrice: 85, standalonePrice: 150, description: "Removes black streaks, mold, and grime from gutter faces and soffits for a clean roofline appearance." },
  { id: "roof-soft-wash", name: "Roof soft wash", addonPrice: 175, standalonePrice: 300, description: "Low-pressure chemical treatment that safely removes moss, algae, and dark streaks from shingles without damage." },
  { id: "deck-patio", name: "Deck & patio restoration", addonPrice: 120, standalonePrice: 200, description: "Deep cleans wood, composite, or concrete deck surfaces and restores them to near-original condition." },
  { id: "window-exterior", name: "Window exterior wash", addonPrice: 70, standalonePrice: 120, description: "Streak-free cleaning of all exterior-facing windows, screens, and frames using purified water." },
  { id: "fence-gate", name: "Fence & gate wash", addonPrice: 95, standalonePrice: 175, description: "Removes green algae, mildew, and weathering from vinyl, wood, or composite fences and gates." },
  { id: "foundation-trim", name: "Foundation & trim brightening", addonPrice: 65, standalonePrice: 110, description: "Targets dirt buildup, efflorescence, and discoloration along the home's foundation and trim lines." },
];
...
const AddOnsList = () => {
  ...
  return (
    <div className="grid sm:grid-cols-2 gap-4">
      {addOns.map((addon, i) => {
        const inCart = items.some((item) => item.id === `standalone-house-${addon.id}`);
        return (
          <button
            key={addon.name}
            type="button"
...
const PWHousePackagesSection = () => {
  ...
  const handleAddPackage = (pkg: Package) => {
    addItem({
      id: `pkg-house-${pkg.name.toLowerCase().replace(/\s/g, "-")}`,
      name: pkg.name,
      price: pkg.price,
      category: "house",
      type: "package",
    ...
    <section id="pw-house-packages" className="py-16 px-6 scroll-mt-16">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-10 rounded-xl overflow-hidden border border-border shadow-lg max-w-3xl mx-auto">
          <video src={houseVideo} autoPlay muted loop playsInline className="w-full h-auto" />
        </div>

        <div ref={ref} className="grid md:grid-cols-3 gap-6">
          {packages.map((pkg) => {
            const inCart = items.some((item) => item.id === `pkg-house-${pkg.name.toLowerCase().replace(/\s/g, "-")}`);
            return (
              <div
                key={pkg.name}
